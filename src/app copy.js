...
(function () {
    ...
    function updateHeaderTitle() {
        ...
        if (printFooter) {
            //printFooter.textContent = userName ? `${userName}` : 'Nombre: ________________________';
        }
    }
    ...
    document.addEventListener('DOMContentLoaded', updateHeaderTitle);
        ...
        const CAREER_ICON_BASE_PATH = '.';
        const DEFAULT_CAREER_ICON_SRC = '../biomedica.png';

        const BASE_CAREER_OPTIONS = [
            { id: 'biomedica', name: 'Biomédica', icon: `${CAREER_ICON_BASE_PATH}../biomedica.png`, fallbackIcon: '../biomedica.png' },
            { id: 'mecatronica', name: 'Mecatrónica', icon: `${CAREER_ICON_BASE_PATH}../meca.png`, fallbackIcon: '../meca.png' },
            { id: 'ambiental', name: 'Ambiental', icon: `${CAREER_ICON_BASE_PATH}../ambiental.png`, fallbackIcon: '../ambiental.png' },
            { id: 'manufactura', name: 'Manufactura', icon: `${CAREER_ICON_BASE_PATH}../manu.png`, fallbackIcon: '../manu.png' },
            { id: 'software', name: 'Software', icon: `${CAREER_ICON_BASE_PATH}../sofware.png`, fallbackIcon: '../sofware.png' },
            { id: 'industrial', name: 'Industrial', icon: `${CAREER_ICON_BASE_PATH}../industrial.png`, fallbackIcon: '../industrial.png' },
            { id: 'petrolera', name: 'Petrolera', icon: `${CAREER_ICON_BASE_PATH}../petro.png`, fallbackIcon: '../petro.png' },
            { id: 'nanotecnologia', name: 'Nanotecnologia', icon: `${CAREER_ICON_BASE_PATH}../nano.png`, fallbackIcon: '../nano.png' },
            { id: 'robotica', name: 'Robótica', icon: `${CAREER_ICON_BASE_PATH}../robotica.png`, fallbackIcon: '../robotica.png' },
            { id: 'alimentos', name: 'Alimentos', icon: `${CAREER_ICON_BASE_PATH}../alimentos.png`, fallbackIcon: '../alimentos.png' },
            { id: 'gestionempresarial', name: 'Gestión Empresarial', icon: `${CAREER_ICON_BASE_PATH}../gestionempresarial.png`, fallbackIcon: '../gestionempresarial.png' },
        ];
        ...
        function closeCareerModal() {
            ...
            modal.classList.add('hidden');
            modal.setAttribute('aria-hidden', 'true');
            const keepNoScroll = document.querySelector('#careerCreateModal:not(.hidden), #careerRenameModal:not(.hidden), #profileModal:not(.hidden), #printMessage:not(.hidden), #driveQuotaModal:not(.hidden), #defaultSchedulePrompt:not(.hidden), #globalLoader:not(.hidden)');
            if (!keepNoScroll) {
        ...
        async function handleCareerCreationSubmit(event) {
            ...
            try {
                await syncDriveAfterChange({ interactive: false, showSuccess: false, silent: true });
            } catch (err) {
                console.warn('No se pudo sincronizar la carrera personalizada en Google Drive', err);
            }
        }
        ...
        async function loadDefaultBiomedicaSchedule() {
            const addedIds = [];
            const missingIds = [];
            ...
            showMessage('Horario ordinario de Biomédica cargado correctamente.', 'success');
            return true;
        }
        ...
        function addInteractionSignInGuard() {
            ...
            const handler = function (e) {
                ...
                try { requestGoogleSignIn(); } catch (ex) { console.warn('requestGoogleSignIn fallo desde guard:', ex); }
            };
        ...
        function loadPredefinedSubjects() {
            catalogSubjects = [
                ...
                },
                {
                    ...
                    aula: "A210",
                    sessions: [
                        { day: "jueves", startTime: "12:00", endTime: "13:45" },
                ...
                },
                {
                    ...
                    aula: "A210",
                    sessions: [
                        { day: "jueves", startTime: "15:00", endTime: "16:45" },
                        { day: "viernes", startTime: "15:00", endTime: "16:45" }
                    ]
                },
        ...
        function updateSignatureLayoutFromOverlay(rect, overlayWidth, overlayHeight) {
            ...
            const scale = reinscriptionPreviewMetrics && reinscriptionPreviewMetrics.scale ? reinscriptionPreviewMetrics.scale : (overlayWidth && reinscriptionPreviewMetrics.baseWidth ? overlayWidth / reinscriptionPreviewMetrics.baseWidth : 1) || 1;
            const baseWidth = reinscriptionPreviewMetrics && reinscriptionPreviewMetrics.baseWidth ? reinscriptionPreviewMetrics.baseWidth : (scale ? overlayWidth / scale : overlayWidth);
            const baseHeight = reinscriptionPreviewMetrics && reinscriptionPreviewMetrics.baseHeight ? reinscriptionPreviewMetrics.baseHeight : (scale ? overlayHeight / scale : overlayHeight);
        ...
        async function renderReinscriptionPreviewCanvas(pdfBytes) {
            ...
            if (canvasMobile && mobileStage) {
                ...
                reinscriptionPreviewMetrics.mobile = {
        ...
        function handleSignaturePointerUp(event) {
            ...
            updateSignatureLayoutFromOverlay(finalRect, overlayWidth, overlayHeight);
        }
        ...
        function isNonAcademicSubject(subject) {
            ...
            // Actualmente sólo Tutorías (id 69)
            return sid === '69';
        }
            ...
            catalogSubjects.forEach(subject => {
                ...
                if (customIds.has(subject.id)) {
                    ...
                    delBtn.addEventListener('click', function (e) {
                        ...
                        // Intentar sincronizar cambios en Drive sin solicitar nuevamente el acceso si no es necesario
                        syncDriveAfterChange({ interactive: false, successMessage: 'Cambio correctamente' })
                            .then(ok => {
                                if (!ok) {
                                    console.warn('No se pudo eliminar');
                                }
                            });
        ...
        function addSubjectToSchedule(id) {
            ...
            // Sincronizar en Drive sin volver a pedir autenticación si la sesión continúa válida.
            (async () => {
                const ok = await syncDriveAfterChange({ interactive: false, successMessage: 'Horario guardado' });
                if (!ok) {
                    console.warn('Error tras añadir materia');
                }
            })();
        ...
        function removeFromSchedule(id) {
            ...
            if (index !== -1) {
                ...
                //   syncDriveAfterChange({ interactive: false, successMessage: 'Cambio guardado en Google Drive' })
                //   .then(ok => {
                //   if (!ok) {
                //      console.warn('Error sincronizando tras quitar materia');
                //      }
                // });
                ...
                for (const hour in timeSlots[day]) {
                    ...
                    if (cell && subjectsInSlot.length > 0) {
                        ...
                        subjectsInSlot.forEach(item => {
                            ...
                            // Aplicar color único para el grupo "7A"
                            if (item.subject.group === "7A") {
        ...
        m_next.addEventListener('click', () => {
            if (!m_inputMateria.value.trim() || !m_inputProfesor.value.trim() || !m_inputGrupo.value.trim()) {
        ...
        s_save.addEventListener('click', () => {
            ...
            // Intentar sincronizar cambios en Drive sin volver a pedir autenticación innecesariamente
            syncDriveAfterChange({ interactive: false, successMessage: 'Materia guardada' })
                .then(ok => {
                    if (!ok) {
                        console.warn('No se pudo guardar nueva materia');
                    }
                });
        ...
        */
        ...
        async function syncDriveAfterChange(options = {}) {
            ...
            try {
                ...
                if (isMissingTokenError(err)) {
                    if (!silent) {
                        showMessage('Debes iniciar sesión con Google para guardar su horario.', 'warning');
                    }
                    return false;
                }
                if (isQuotaExceededError(err)) {
        ...
        async function loadFromDrive() {
            showLoader('Cargando datos desde Google Drive...');
            try {
        ...
        async function onSignedIn() {
            ...
            try {
                ...
                if (existing) {
                    // Si hay datos locales guardados por el usuario, preservarlos y subirlos a Drive
                    ...
                    if (Array.isArray(localStored) && localStored.length > 0) {
                        ...
                        // aplicar local a memoria y sincronizar hacia Drive
                        ...
                        // actualizar vistas con los datos locales
                        updateScheduleView();
                        updateSelectedSubjectsList();
                        updateReinscriptionLoadedSubjectsList();
                    } else {
                        // Cargar automáticamente desde Drive si no hay datos locales
                        console.log('Cargando horario automáticamente desde Google Drive...');
                        await loadFromDrive();
                    }
                } else {
                    maybePromptDefaultSchedule();
        ...
        document.addEventListener('DOMContentLoaded', () => {
                        ...
                        try {
                            (async () => {
                                ...
                                if (restored) {
                                    ...
                                    // La sesión previa expiró: limpiar horario local para evitar datos obsoletos
                                    try {
                                        clearLocalScheduleAndCatalogOnSignOut();
                                    } catch (e) { console.warn('Error limpiando horario tras sesión expirada', e); }
                                    // Activar guard que obliga a iniciar sesión en el siguiente gesto del usuario
                                    requireSignInOnInteraction = true;
                                    try { addInteractionSignInGuard(); } catch (e) { console.warn('No se activó guard de interacción:', e); }
                                    notifySessionExpired();
                                    console.log('La sesión previa expiró; horario local limpiado. El usuario debe volver a iniciar sesión.');
                                } else {
            ...
            if (defaultAccept) {
                defaultAccept.addEventListener('click', async () => {
                    defaultSchedulePromptDismissed = false;
                    closeDefaultSchedulePrompt();
                    showLoader('Cargando horario de Biomédica...');
                    const delay = new Promise(resolve => setTimeout(resolve, 5000));
                    try {
                        await Promise.all([
                            loadDefaultBiomedicaSchedule(),
                            delay
                        ]);
                    } finally {
                        hideLoader();
            ...
            if ((status === 401) || (status === 403 && !isQuotaExceededError(err)) || unauthorizedMsg) {
                ...
                try {
                    const msgText = willAutoRetry
                        ? 'Se requiere inicio de sesión. Abriendo la ventana de inicio de sesión...'
                        : 'Se requiere inicio de sesión para guardar su horario. Haz clic en "Iniciar sesión" para continuar.';
                    showMessage(msgText, 'warning');
                } catch (e) { }
                    ...
                    setTimeout(() => {
                        if (typeof requestGoogleSignIn === 'function') {
                            try {
                                ...
                                console.warn('No se pudo iniciar reautenticación automática', autoErr);
                                driveAuthRetryPending = false;
                            }
                        }
                    }, 350);
