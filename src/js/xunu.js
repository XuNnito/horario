...
function updateScheduleView() {
    ...
    const customIds = new Set(loadCustomSubjects().map(c => c.id));
        ...
        for (const hour in timeSlots[day]) {
            ...
            if (cell && subjectsInSlot.length > 0) {
                ...
                subjectsInSlot.forEach(item => {
                    ...
                    if (customIds.has(item.subject.id)) {
                        const editButton = document.createElement('button');
                        editButton.type = 'button';
                        editButton.className = 'edit-from-schedule';
                        editButton.textContent = '✎';
                        editButton.title = 'Editar materia';
                        editButton.setAttribute('aria-label', `Editar ${item.subject.name}`);
                        editButton.addEventListener('click', function (e) {
                            e.stopPropagation();
                            openCustomSubjectEditor(item.subject.id);
                        });
                        subjectCard.appendChild(editButton);
                    }
