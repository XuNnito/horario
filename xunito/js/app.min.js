...
function addSubjectToSchedule(id) {
    ...
    // Guardar el horario inmediatamente en Drive (pedirá inicio de sesión si hace falta)
    (async () => {
        try {
            const ok = await ensureSaveToDrive({ interactive: true, successMessage: 'Horario guardado' });
            if (!ok) {
                console.warn('Error tras añadir materia al guardar en Drive');
            }
        } catch (e) {
            console.warn('Error tras añadir materia al intentar guardar en Drive', e);
        }
    })();
...
s_save.addEventListener('click', () => {
    ...
    // Guardar inmediatamente en Drive y, si hace falta, pedir inicio de sesión
...
async function loadFromDrive(isAuto = false) {
    if (!isAuto) {
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Sincronización periódica (entre dispositivos): revisar cada cierto tiempo
    // si el archivo en Drive fue modificado por otra sesión.
    try {
        if (!window.__driveAutoSyncIntervalId) {
            window.__driveAutoSyncIntervalId = setInterval(async () => {
                ...
                try {
                    ...
                    // Primera detección del archivo en esta sesión: cargarlo desde Drive
                    // para que este dispositivo se alinee con la versión remota.
