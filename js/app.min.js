...
async function saveToDrive(options = {}) {
    ...
    try {
        ...
        if (existing) {
            ...
            try {
                // Actualizar marca local de última modificación conocida en Drive
...
async function loadFromDrive(isAuto = false) {
    ...
    try {
        ...
        if (data) {
            try {
                // Guardar la marca de última modificación conocida en Drive
...
function describePlan(planId) {
    ...
    // "plus_30" es nuestro plan de 49 MXN / 5 días
    return 'Plan 5 días';
}
...
function renderPlanInProfile() {
    ...
    if (currentPlanState.planId !== 'free' && currentPlanState.expiresAtTs && currentPlanState.nowTs) {
        var diffSec = currentPlanState.expiresAtTs - currentPlanState.nowTs;
        var remainingDays = Math.max(0, Math.ceil(diffSec / (24 * 60 * 60)));
        if (remainingDays > 0) {
            pmPlan.textContent = label + ' (' + remainingDays + ' día' + (remainingDays === 1 ? '' : 's') + ' restantes)';
            return;
        }
    }
...
async function startPlanCheckout(planId) {
    var email = (typeof getCurrentUserEmail === 'function') ? getCurrentUserEmail() : '';
    var name = (typeof getCurrentUserFullName === 'function') ? getCurrentUserFullName() : '';
    if (!email) {
        if (typeof showMessage === 'function') {
            showMessage('Inicia sesión con Google antes de comprar un plan.', 'warning');
        }
        return;
    }

    var url = apiUrl('/api/checkout/create-session');
    var res;
    try {
        res = await fetch(url, {
            method: 'POST',
    ...
    try {
        ...
        console.error('redirectToCheckout falló', e);
        if (typeof showMessage === 'function') {
            showMessage('No se pudo redirigir a la página de pago.', 'error');
        }
    }
}
