...
function describePlan(planId) {
    ...
    // "Plan xunu" es nuestro plan de 49 MXN basado en usos
    return 'Plan 49 MXN';
}
    ...
    if (!__payerModalInitialized) {
        ...
        if (__payerContinueBtn) {
            __payerContinueBtn.addEventListener('click', async function () {
                ...
                try {
                    await startPlanCheckout('Plan xunu', email, name);
                    closePayerInfoModal();
                } finally {
...
async function handleConfirmPaymentClick() {
    ...
    if (result && result.paymentIntent && result.paymentIntent.status === 'succeeded') {
        ...
        try {
            // Sabemos que este flujo solo vende el plan de 49 MXN (Plan xunu)
            if (typeof currentPlanState !== 'undefined' && currentPlanState) {
                currentPlanState.planId = 'Plan xunu';
                currentPlanState.rawPlan = 'Plan xunu';
            ...
            try {
                ...
                if (sessionEmail) {
                    ...
                    await fetch(activateUrl, {
                        ...
                        headers: { 'Content-Type': 'application/json' },
                        credentials: 'include',
                        body: JSON.stringify({ email: sessionEmail, name: sessionName, plan_id: 'Plan xunu' })
