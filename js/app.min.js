...
const CAREER_ICON_BASE_PATH = 'https://horariobio.netlify.app/';
const fallbackIconimg = 'https://horariobio.netlify.app/';
...
async function loadFromDrive(isAuto = false) {
    ...
    try {
        ...
        if (data) {
            ...
            try {
                if (data._meta && data._meta.planId) {
                    currentPlanState.planId = data._meta.planId || 'free';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var shareUrl = 'https://horariobio.netlify.app/';
...
function describePlan(planId) {
    ...
    // "plus_30" es nuestro plan de 49 MXN basado en usos
    return 'Plan 49 MXN';
}
    ...
    if (!__payerModalInitialized) {
        ...
        if (__payerContinueBtn) {
            __payerContinueBtn.addEventListener('click', async function () {
                ...
                try {
                    await startPlanCheckout('plus_30', email, name);
                    closePayerInfoModal();
                } finally {
...
async function handleConfirmPaymentClick() {
    ...
    if (result && result.paymentIntent && result.paymentIntent.status === 'succeeded') {
        ...
        try {
            // Sabemos que este flujo solo vende el plan de 49 MXN (plus_30)
            if (typeof currentPlanState !== 'undefined' && currentPlanState) {
                currentPlanState.planId = 'plus_30';
                currentPlanState.rawPlan = 'plus_30';
            ...
            try {
                ...
                if (sessionEmail) {
                    ...
                    await fetch(activateUrl, {
                        ...
                        headers: { 'Content-Type': 'application/json' },
                        credentials: 'include',
                        body: JSON.stringify({ email: sessionEmail, name: sessionName, plan_id: 'plus_30' })
...
(function () {
  ...
  console.log(
    ...
    "es un fraude. Si lo haces, XuNnito podrían robar tu cuenta.",
    estiloTexto
  );
})();
